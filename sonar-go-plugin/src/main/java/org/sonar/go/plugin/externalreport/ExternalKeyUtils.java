...
public class ExternalKeyUtils {
  ...
  static final List<ExternalKey> GO_LINT_KEYS = Arrays.asList(
          new ExternalKey("AllCaps", msg -> msg.contains("don't use ALL_CAPS in Go names; use CamelCase")),
          ...
          new ExternalKey("BlankImport", msg -> msg.contains("a blank import should be only in a main or test package, or have a comment justifying it")),
          new ExternalKey("CommentExportedType", msg ->
                  msg.matches("[0-9]+: comment on exported type [a-zA-Z0-9]+ should be of the form \"[a-zA-Z0-9]+ ...\" \\\\(with optional leading article\\\\)")),
          new ExternalKey("ContextContextFirstArg", msg -> msg.contains("context.Context should be the first parameter of a function")),
          new ExternalKey("DropAssignment", msg -> msg.contains("from declaration of var") && msg.endsWith("it is the zero value")),
          new ExternalKey("ErrorEndString", msg -> msg.contains("error strings should not be capitalized or end with punctuation or a newline")),
          new ExternalKey("ErrorLastReturn", msg -> msg.contains("error should be the last type when returning multiple items")),
          new ExternalKey("ErrorVarName", msg -> msg.matches("[0-9]+: error var [a-zA-Z0-9]+ should have name of the form [a-zA-Z0-9]+")),
          new ExternalKey("ExportedDeclaration", msg -> msg.endsWith("should have its own declaration")),
          new ExternalKey("ExportedHaveComment", msg -> msg.matches("[0-9]+: exported [a-zA-Z0-9\\.]+ [a-zA-Z0-9\\.]+ should have comment[\\(\\) a-zA-Z0-9]* or be unexported")),
          new ExternalKey("FormComment", msg -> msg.contains("comment on exported")),
          new ExternalKey("HavePackageComment", msg -> msg.contains("should have a package comment, unless it's in another file for this package")),
          ...
          new ExternalKey("Initialisms", msg -> msg.matches("[0-9]+: (range var|[a-zA-Z0-9]+) [a-zA-Z0-9]+ should be [a-zA-Z0-9]+")),
          new ExternalKey("LeadingK", msg -> msg.contains("don't use leading k in Go names;")),
          new ExternalKey("NotUseBasicType", msg -> msg.contains("should not use basic type")),
          new ExternalKey("NotUseDotImport", msg -> msg.contains("should not use dot imports")),
          new ExternalKey("OmitType", msg -> msg.contains("should omit type") && msg.endsWith("it will be inferred from the right-hand side")),
          new ExternalKey("PackageCommentForm", msg -> msg.contains("package comment should be of the form")),
          new ExternalKey("PackageCommentSpace", msg -> msg.contains("package comment should not have leading space")),
          new ExternalKey("PackageCommentDetached", msg -> msg.contains("package comment is detached; there should be no blank lines between it and the package statement")),
          new ExternalKey("PackageNames", msg ->
                  msg.matches("[0-9]+: +[a-zA-Z0-9]+ name will be used as [a-zA-Z0-9]+.[a-zA-Z0-9]+ by other packages, and that stutters; consider calling this [a-zA-Z0-9]+")),
          new ExternalKey("PackageNamesMixedCaps", msg -> msg.contains("don't use MixedCaps in package name;")),
          new ExternalKey("ReceiverNameConsistent", msg -> msg.contains("should be consistent with previous receiver name")),
          new ExternalKey("ReceiverNameNotUnderscore", msg -> msg.contains("receiver name should not be an underscore")),
          new ExternalKey("ReceiverNameReflection", msg ->
                  msg.contains("receiver name should be a reflection of its identity; don't use generic names such as \"this\" or \"self\"")),
          new ExternalKey("ReplaceLintIncDec", msg -> msg.matches("[0-9]+: should replace [a-zA-Z0-9\\-\\+=\\.\\(\\) ]+ with [a-zA-Z0-9\\-\\+\\.\\(\\)]+")),
          new ExternalKey("ReplaceSprintf", msg -> msg.matches("[0-9]+: should replace [a-zA-Z0-9\\.]+\\\\(fmt.Sprintf\\\\(...\\\\)\\\\) with [a-zA-Z0-9]+.Errorf\\\\(...\\\\)")),
          new ExternalKey("RangeLoop", msg -> msg.matches("[0-9]+: should omit (2nd value|values) from range; this loop is equivalent to .*")),
          new ExternalKey("SpecificSuffix", msg -> msg.contains("don't use unit-specific suffix")),
          new ExternalKey("StructFieldInitialisms", msg -> msg.matches("[0-9]+: struct field [a-zA-Z0-9]+ should be [a-zA-Z0-9]+")),
          new ExternalKey("UnderscoreInGoName", msg -> msg.contains("don't use underscores in Go names;")),
          new ExternalKey("UnderscoreInPackageName", msg -> msg.contains("don't use an underscore in package name"))
  );
