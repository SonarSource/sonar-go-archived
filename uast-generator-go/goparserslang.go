...
func convert(astNode ast.Node) *SlangNode {
	switch node := astNode.(type) {
	case *ast.IfStmt:
		return createIfTree(node)
	default:
...
func createNativeTree(astNode ast.Node) *SlangNode {
	metaData := createMetaData(astNode)
...
func createMetaData(astNode ast.Node) *TreeMetaData {
